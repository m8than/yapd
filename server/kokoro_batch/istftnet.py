...
class SineGen(nn.Module):
    ...
    def _f02sine(self, f0_values):
        """Convert batched F0 harmonics to phase-continuous sine waves."""
        rad_values = (f0_values / self.sampling_rate) % 1
        rand_ini = torch.rand(
            f0_values.shape[0], f0_values.shape[2],
            device=f0_values.device, dtype=f0_values.dtype,
        )
        rand_ini[:, 0] = 0
        ...
        return torch.cos(i_phase * 2 * torch.pi)
    ...
    def forward(self, f0):
        ...
        )  # b x length x dim = num_harmonics
...
class Generator(nn.Module):
    ...
    def forward(self, x, s, f0): # x: b x 512 x frames*2, s: b x 1 x sty_dim, f0: b x 1 x frames
        with torch.no_grad():
            ...
            har = torch.cat([har_spec, har_phase], dim=1).to(dtype=x.dtype)
        ...
        return self.stft.inverse(spec.float(), phase.float())  # b x T
...
class AdainResBlk1d(nn.Module):
    ...
    def forward(self, x, s, frame_mask): # x: b x d_model x max_dur, s: b x 1 x sty_dim
        ...
        out = (out + self._shortcut(x, upsampled_mask)) * (2 ** -0.5) * upsampled_mask
